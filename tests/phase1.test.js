...
const withMode = (mode, overrides = {}) =>
  resolveConfig({ mode, seed: SEED, episodes: EPISODES, ...overrides });
...
describe("phase 1 simulation", () => {
  ...
  it("control: thetaA stays 1000 and no bucket evolves over 300 episodes", () => {
  ...
  it("censoring: a cleared escalation is not evidence, so thetaA stays 1000", () => {
  ...
  it("drift: the habit marches past $5,000", () => {
    const m = runSimulation(withMode("drift")).metrics;
    // Measured with this seed: thetaA 1000 -> 6000, first >= 5000 at id 206.
  ...
  it("fraud: complaints fire and the run is bounded relative to drift", () => {
    ...
    // Measured with this seed: 4 fraud auto-approvals -> 4 complaints, and
    // thetaA ends at 4500 rather than drift's 6000.
    expect(fraud.metrics.complaints).toBeGreaterThan(0);
    expect(fraud.metrics.fraudAutos).toBe(fraud.metrics.complaints);
    expect(fraud.metrics).not.toEqual(drift.metrics);
